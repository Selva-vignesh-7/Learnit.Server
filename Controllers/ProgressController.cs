...
namespace Learnit.Server.Controllers
{
    ...
    public class ProgressController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetProgressDashboard()
        {
            ...
            // Calculate current week boundaries (Monday to Sunday) - consistent with frontend
            var today = DateTime.UtcNow.Date;
            var dayOfWeek = (int)today.DayOfWeek; // 0 = Sunday, 1 = Monday, etc.
            var daysFromMonday = dayOfWeek == 0 ? 6 : dayOfWeek - 1; // Convert Sunday (0) to 6
            var weekStart = today.AddDays(-daysFromMonday); // Monday of current week
            ...
            var allUserEvents = await _db.ScheduleEvents
                .Include(e => e.CourseModule)
                .Where(e => e.UserId == userId &&
                           // Fetch events that might overlap: start before weekEnd OR end after weekStart
                           (e.StartUtc < weekEnd || (e.EndUtc.HasValue && e.EndUtc.Value > weekStart)))
                .ToListAsync();
            
            // Filter to only events that actually overlap with the week
            var weeklyEvents = allUserEvents.Where(e =>
            {
                var eventStart = e.StartUtc;
                var eventEnd = e.EndUtc ?? e.StartUtc.AddHours(1);
                // Event overlaps with week if: eventEnd > weekStart AND eventStart < weekEnd
            ...
            var weeklySessions = await _db.StudySessions
                ...
                .Where(s => s.IsCompleted && 
                           s.StartTime.Date >= weekStart && 
                           s.StartTime.Date < weekEnd)
                .GroupBy(s => s.StartTime.Date)
                .Select(g => new { Date = g.Key, Hours = g.Sum(s => s.DurationHours) })
                .ToDictionaryAsync(k => k.Date, v => v.Hours);
            ...
            for (int i = 0; i < 7; i++)
            {
                ...
                var dayStart = date; // Start of this day
                var dayEnd = date.AddDays(1); // Start of next day (exclusive)
                ...
                var scheduled = weeklyEvents
                    ...
                    .Sum(e =>
                    {
                        ...
                        var eventStartInDay = eventStart < dayStart ? dayStart : eventStart;
                        var eventEndInDay = eventEnd > dayEnd ? dayEnd : eventEnd;
                ...
                var completed = weeklySessions.TryGetValue(date, out var sessionHours) 
                    ? sessionHours 
                    : weeklyEvents
                    .Where(e =>
                    {
                    ...
                    .Sum(e =>
                    {
                        ...
                        var isDone = e.CourseModule?.IsCompleted == true || eventEnd <= DateTime.UtcNow;
                        return isDone ? hours : 0;
                    });

                // Format day label: "Mon", "Tue", etc., with "Today" for today
                var dayLabel = date == today ? "Today" : date.ToString("ddd");
        ...
        private async Task<int> CalculateLongestStreak(int userId)
        {
            ...
            // This data is PERSISTENT in the database and will survive logouts
            var completedSessions = await _db.StudySessions
                .Join(_db.Courses.Where(c => c.UserId == userId), s => s.CourseId, c => c.Id,
                    (s, _) => s)
                .Where(s => s.IsCompleted) // Only count completed sessions (persisted in DB)
                .Select(s => s.StartTime.Date)
        ...
        private async Task<List<int>> GenerateHeatmapData(int userId)
        {
            ...
            var activityLogsList = await _db.ActivityLogs
                .Where(a => a.UserId == userId && a.Date >= startDate && a.Date <= today)
                .ToListAsync();

            Dictionary<DateTime, decimal> dailyHours;
            if (activityLogsList.Any())
            {
                var activityLogs = activityLogsList.ToDictionary(a => a.Date.Date, a => a.HoursCompleted);
            ...
            else
            {
                ...
                var sessions = await _db.StudySessions
                    .Join(_db.Courses.Where(c => c.UserId == userId), s => s.CourseId, c => c.Id,
                        (s, _) => s)
                    .Where(s => s.IsCompleted && s.StartTime.Date >= startDate && s.StartTime.Date <= today)
                    .GroupBy(s => s.StartTime.Date)
                    .Select(g => new { Date = g.Key, Hours = g.Sum(s => s.DurationHours) })
                    .ToDictionaryAsync(k => k.Date, v => v.Hours);
